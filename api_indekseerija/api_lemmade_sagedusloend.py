...
class IDX_STAT_LEMMAS:
    ...
    def leia_koik_lemmad(self, json_io:List, sorted_by_lemma:bool, sortorder_reverse:bool) -> List:
        try:
        ...
        for token in json_io["annotations"]["tokens"]:  # tsükkel üle sõnede
            lemmavariandid = []                     # selle massivi abil hoimae meeles, millesed lemma kujud olema juba lisanud
            for mrf in token["features"]["mrf"]:            # tsükkel üle sama sõne alüüsivariantide
                if self.ignore_pos.find(mrf["pos"]) != -1:      # selle sõnaliiiga lemmasid...
                    continue                                    # ...ignoreerime, neid ei indekseeri
                if mrf["lemma_ma"] in lemmavariandid:   # sõne morf analüüside hulgas võib sama kujuga lemma erineda ainult käände/põõrde poolest
                    continue                                    # ei lisa sama kujuga lemmat mitu korda
                lemmavariandid.append(mrf["lemma_ma"])  # jätame meelde, et sellise lemma lisame
            # lemmavariandid[] -- jooksva sõne unikaalsed lemmad
            for lemma in lemmavariandid:
                puhas_lemma = lemma.replace('_', '').replace('=', '')
                if puhas_lemma not in stat_dct:
                    stat_dct[puhas_lemma] = 1                 # selline lemma esines tekstis 1. korda
                else:
                    stat_dct[puhas_lemma] += 1                # selline lemma juba oli tekstis, suurendame loendajat

        # teeme sõnastikust listi...
        for d in stat_dct:
            stat_list.append([d, stat_dct[d]])
...
if __name__ == '__main__':
    ...
    if args.json is not None:
        ...
        if "error" in json_io:
            ...
            sys.exit(1)
        if args.cvs is True:
            stat_list = IDX_STAT_LEMMAS().leia_koik_lemmad(json_io, args.sorted_by_lemma, args.reverse)
            for i in stat_list:
                sys.stdout.write(f'{i[0]}\t{i[1]}\n')
        else:
            json.dump(IDX_STAT_LEMMAS().leia_koik_lemmad(json_io, args.sorted_by_lemma, args.reverse), sys.stdout, indent=args.indent, ensure_ascii=False)
            sys.stdout.write('\n')
    else:
